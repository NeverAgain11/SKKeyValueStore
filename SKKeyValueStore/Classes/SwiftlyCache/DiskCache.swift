...
/**
 支持for...in循环
 */
public class DiskCacheGenerator: IteratorProtocol {
    public typealias Element = (key: String, object: Codable)
...
//        if let element = diskCache.object(forKey: key, objectType: objectType) {
//            return (key, element)
//        }
        return nil
    }
    
    fileprivate init(diskCache: DiskCache) {
...
public class ConvertibleFactory {
    ...
    public func toData<T: Codable>(value: T)throws -> Data? {
        let data = try? JSONEncoder().encode(value)
        return data
    }
    
    public func fromData<Value: Codable>(data: Data, objectType: Value.Type) throws -> Value? {
...
public class DiskCache {
    ...
    private let convertible: ConvertibleFactory = ConvertibleFactory()
...
extension DiskCache: Sequence {
     ...
     */
    public subscript<Value: Codable>(key: String, objectType: Value.Type) -> Value? {
        set {
            if let newValue = newValue { set(object: newValue, forKey: key) }
        } get {
            if let object = object(forKey: key, objectType: objectType) { return object }
            return nil
        }
    }
...
extension DiskCache {
    ...
    public func object<Value: Codable>(forKey key: String, objectType: Value.Type) -> Value? {
        ...
        do {
            let object = try convertible.fromData(data: data, objectType: objectType)
            return object
        } catch {
    ...
    public func object<Value: Codable>(forKey key: String, objectType: Value.Type, completionHandler: @escaping((_ key: String, _ value: Value?) -> Void)) {
        queue.async {
            if let object = self.object(forKey: key, objectType: objectType) { completionHandler(key, object) } else { completionHandler(key, nil) }
        }
    }
